...
export const encounterStatusSchema = z.enum(['draft', 'active', 'completed', 'archived'], {
  errorMap: () => ({ message: 'Invalid encounter status' }),
});

// Encounter difficulty validation
export const encounterDifficultySchema = z.enum(['trivial', 'easy', 'medium', 'hard', 'deadly'], {
...
export const participantReferenceSchema = z.object({
  ...
  name: z.string().min(1, 'Participant name is required').max(100, 'Name cannot exceed 100 characters'),
  ...
  conditions: createArraySchema(
    z.string().min(1, 'Condition name is required').max(50, 'Condition name cannot exceed 50 characters'),
    0,
    20
  ).default([]),
  position: createOptionalSchema(z.object({
...
export const encounterSettingsSchema = z.object({
  ...
  gridSize: z.number().min(1, 'Grid size must be at least 1').max(50, 'Grid size cannot exceed 50').default(5),
  roundTimeLimit: createOptionalSchema(z.number().min(30, 'Round time limit must be at least 30 seconds').max(600, 'Round time limit cannot exceed 10 minutes')),
...
export const combatStateSchema = z.object({
  isActive: z.boolean().default(false),
  currentRound: z.number().int('Round must be a whole number').min(0, 'Round cannot be negative').default(0),
  currentTurn: z.number().int('Turn must be a whole number').min(0, 'Turn cannot be negative').default(0),
...
export const encounterSchema = z.object({
  ...
  name: z.string().min(1, 'Encounter name is required').max(100, 'Name cannot exceed 100 characters'),
  description: z.string().max(1000, 'Description cannot exceed 1000 characters').default(''),
  tags: createArraySchema(
    z.string().min(1, 'Tag cannot be empty').max(30, 'Tag cannot exceed 30 characters'),
    0,
    10
  ).default([]),
  difficulty: createOptionalSchema(encounterDifficultySchema),
  estimatedDuration: createOptionalSchema(z.number().min(1, 'Duration must be at least 1 minute').max(480, 'Duration cannot exceed 8 hours')),
  ...
  version: z.number().int('Version must be a whole number').min(1, 'Version must be at least 1').default(1),
...
export const encounterSummarySchema = encounterSchema.pick({
  _id: true,
  name: true,
  description: true,
  tags: true,
  ...
  participantCount: z.number().int('Participant count must be a whole number').min(0, 'Participant count cannot be negative'),
  playerCount: z.number().int('Player count must be a whole number').min(0, 'Player count cannot be negative'),
  isActive: z.boolean(),
});
...
export const damageParticipantSchema = z.object({
  encounterId: objectIdSchema,
  participantId: objectIdSchema,
  damage: z.number().int('Damage must be a whole number').min(0, 'Damage cannot be negative'),
  damageType: createOptionalSchema(z.string().max(50, 'Damage type cannot exceed 50 characters')),
});

export const healParticipantSchema = z.object({
  encounterId: objectIdSchema,
  participantId: objectIdSchema,
  healing: z.number().int('Healing must be a whole number').min(0, 'Healing cannot be negative'),
});

export const addConditionSchema = z.object({
  encounterId: objectIdSchema,
  participantId: objectIdSchema,
  condition: z.string().min(1, 'Condition name is required').max(50, 'Condition name cannot exceed 50 characters'),
  duration: createOptionalSchema(z.number().int('Duration must be a whole number').min(1, 'Duration must be at least 1 round')),
});
...
export const searchEncountersSchema = z.object({
  query: z.string().min(1, 'Search query is required').max(100, 'Query cannot exceed 100 characters'),
  difficulty: createOptionalSchema(encounterDifficultySchema),
  targetLevel: createOptionalSchema(levelSchema),
...
export type ParticipantReference = InferSchemaType<typeof participantReferenceSchema>;
...
export type EncounterDifficulty = InferSchemaType<typeof encounterDifficultySchema>;
...
export type GetEncountersByOwner = InferSchemaType<typeof getEncountersByOwnerSchema>;
